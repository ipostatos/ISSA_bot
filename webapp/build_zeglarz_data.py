"""
Экспорт банка Żeglarz jachtowy в статический JS для Mini App «Лицензии».

questions_zeglarz.json  ->  webapp/zeglarz_data.js  (глобал window.ZEGLARZ_DATA)
...
Перегенерация после изменения банка:
    python webapp/build_zeglarz_data.py
"""
...
SRC = Path(__file__).resolve().parent / "questions_zeglarz.json"
OUT = Path(__file__).resolve().parent / "zeglarz_data.js"

# Параметры экзамена Żeglarz jachtowy (PZŻ): 75 вопросов, 65 верных, 90 мин.
EXAM_SIZE = 75
EXAM_PASS = 65
EXAM_MINUTES = 90
...
def main() -> None:
    ...
    assert len(ids) == len(set(ids)), "дубли id в questions_zeglarz.json"
    ...
    js = (
        ...
        "// Источник: questions_zeglarz.json. Перегенерация: python webapp/build_zeglarz_data.py\n"
        "window.ZEGLARZ_DATA = "
