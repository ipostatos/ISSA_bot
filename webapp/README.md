...
| `tasks.html` | 📝 Задачки: S-D-T/ETA и приливы с проверкой | `/tasks.html` |
| `glossary.html` | 📚 Словарь: 180+ терминов, живой поиск и категории | `/glossary.html` |
...
```bash
WEBAPP_HOME_URL=https://issa-46-224-220-94.sslip.io
WEBAPP_URL=https://issa-46-224-220-94.sslip.io/calc.html
WEBAPP_QUIZ_URL=https://issa-46-224-220-94.sslip.io/quiz.html
```

Когда задан `WEBAPP_HOME_URL`, главное меню бота показывает кнопку
**«🚀 Открыть приложение»**, а контентные разделы (конспект, шпаргалки,
словарь, калькулятор, задачи) убираются из чат-меню как дубли приложения —
в чате остаётся то, что завязано на прогресс (тесты, экзамен, работа над
ошибками, статистика). Если переменная не задана — показывается полное
меню со всеми разделами в чате (ничего не теряется).
